...
import static org.opencv.imgcodecs.Imgcodecs.IMREAD_REDUCED_GRAYSCALE_8;
...
public class CVCore {
    ...
    public int[] attachData(byte[] byteData, int outputType) {
        int[] byteArray1 = null;
        try {
            // Decode image from input byte array
           Mat src = Imgcodecs.imdecode(new MatOfByte(byteData), Imgcodecs.IMREAD_UNCHANGED);
            // Mat src = Imgcodecs.imread(filename);
//            Imgcodecs.imread(filename, Imgcodecs.IMREAD_REDUCED_GRAYSCALE_8);
            if (src == null) return byteArray1;
...
//            ImgHash
            BlockMeanHash algorithm = BlockMeanHash.create(0);
            algorithm.compute(src, dst);

            byte[] byteArray = new byte[(int)(dst.total() * dst.channels())];

            dst.get(0,0, byteArray);
...
//            byteArray = matOfByte.toArray();
            ...
            IntBuffer intBuf =
                    ByteBuffer.wrap(byteArray)
                            ...
                            .asIntBuffer();
            byteArray1 = new int[intBuf.remaining()];
            intBuf.get(byteArray1);
            src.release();
            dst.release();
        } catch (Exception e) {
