...
import 'package:communitydownloader/component/postprocessor/ugoira.dart';
...
class PixivAPI {
  static Future<String> getAccessToken(String username, String password) async {
    ...
    var result = await HttpWrapper.post(
  ...
  static Future<Map<String, dynamic>> getUser(
    ...
    var result = await HttpWrapper.getr(
        url +
            '?' +
  ...
  static Future<List<dynamic>> getWorks(
    ...
    return jsonDecode(utf8.decode(result.bodyBytes))['response'];
  }
...
class PixivManager extends Downloadable {
  ...
  Future<List<DownloadTask>> createTask(
    ...
    if (match.first[2].startsWith('member') ||
      ...
      for (int i = 0; i < works.length; i++) {
        try {
          ...
          if (e['type'] == null || e['type'] == 'illustration') {
            ...
            var ugoiraData =
                await PixivAPI.getUgoira(accessToken, e['id'].toString());
            var task = DownloadTask(
              url: ugoiraData['zip_urls']['medium'],
              filename: ugoiraData['zip_urls']['medium'].split('/').last,
              referer: url,
              format: FileNameFormat(
                artist: user['name'],
                account: user['account'],
                id: user['id'].toString(),
                filenameWithoutExtension: path.basenameWithoutExtension(
                    ugoiraData['zip_urls']['medium'].split('/').last),
                extension: path
                    .extension(ugoiraData['zip_urls']['medium'].split('/').last)
                    .replaceAll(".", ""),
                extractor: 'pixiv',
              ),
              postprocessorTask: PostprocessorTask(
                postprocessor: UgoiraPostprocessor(
                  frames: ugoiraData['frames'],
                ),
              ),
            );
            task.postprocessorTask.downloadTask = task;
      ...
      return result;
    } else if (match.first[2].startsWith('artworks')) {
      var result = List<DownloadTask>();
      var works = await PixivAPI.getWorks(
          accessToken, int.parse(match.first.namedGroup('id')));
      var thumbnail = false;

      for (int i = 0; i < works.length; i++) {
        try {
          var user = works[0]['user'];
          var e = works[i];
          if (i == 0) {
            if (gdp.simpleInfoCallback != null)
              gdp.simpleInfoCallback.call('${e['title']}');
          }
          if (e['page_count'] > 1) {
            e['metadata']['pages'].forEach((e) {
              result.add(
                DownloadTask(
                  url: e['image_urls']['large'],
                  filename: e['image_urls']['large'].split('/').last,
            ...
            if (!thumbnail && result.length > 0) {
              if (gdp.thumbnailCallback != null)
                gdp.thumbnailCallback
                    .call(result[0].url, jsonEncode({'Referer': url}));
            }
          }
  ...
  Future<bool> tryLogin() async {
    ...
    var id = await storage.read(key: 'pixiv_id');
    var pwd = await storage.read(key: 'pixiv_pwd');
